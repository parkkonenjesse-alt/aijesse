    ...
    function renderCalendar(root) {
      ...
      function formStep() {
        root.innerHTML = '<button class="bk-back" data-back>‹ Takaisin</button><span class="bk-sel-label">'+dstr(sel)+' klo '+time+'</span>' +
          ...
          '<input type="text" data-company tabindex="-1" autocomplete="off" aria-hidden="true" style="position:absolute;left:-9999px;width:1px;height:1px;opacity:0">' +
        ...
        root.querySelector('[data-confirm]').addEventListener('click', () => {
          ...
          const payload = { name: name, email: email, note: note, date: dstr(sel)+' '+sel.getFullYear(), time: time, company: (root.querySelector('[data-company]').value||'') };
          const btn = root.querySelector('[data-confirm]'); btn.style.pointerEvents = 'none'; const lbl = btn.querySelector('.c-button_label'); if (lbl) lbl.textContent = 'Lähetetään…';
          fetch('/api/booking', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(r => { if (r.ok) { doneStep(g, 'sent'); } else { window.location.href = mail; doneStep(g, 'mailto'); } })
            .catch(() => { window.location.href = mail; doneStep(g, 'mailto'); });
        });
      }
      function doneStep(gLink, mode) {
        const msg = mode === 'sent'
          ? 'Varauspyyntö lähetetty: '+dstr(sel)+' klo '+time+'. Vahvistus tulee sähköpostiisi.'
          : 'Sähköpostiohjelmasi avautui valmiilla viestillä, '+dstr(sel)+' klo '+time+'. Lähetä se, niin vahvistan ajan pian.';
        root.innerHTML = '<div class="bk-success"><div class="bk-success__check">✓</div>' +
          ...
          '<p style="opacity:.82;font-size:.9rem;line-height:1.6;margin-bottom:1.2rem">'+msg+'</p>' +
